...
{#each content as card, index}
  <SwiperSlide>
    ...
    <div class="exposition-slide {card.center ? 'flex-center' : 'flex-end'}">
      <div
        bind:clientHeight={card["height"]}
        class="slide-content {firstSlide ? 'first-slide' : ''}"
        style={thirdSlide ? "padding-bottom: 20%" : ""}
      >
      ...
      {#if card.img}
        ...
        >
          <img
            src="assets/{viewportWidth < 700 ? card.imgmobi : card.img}"
            alt="global-earth-human"
          />
        </div>
      {/if}
    </div>
